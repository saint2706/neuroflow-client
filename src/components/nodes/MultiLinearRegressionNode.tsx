...
const MultiLinearRegressionNode = ({ id, data, isConnectable }) => {
  ...
  const getCollapsedSummary = () => {
    if (modelResults) {
      ...
      const xCount = selectedX.length;
      return `Trained | R²: ${r2}% | ${xCount} vars → ${yCol}`;
    }
    if (selectedX.length > 0 && yCol) {
      return `Configured | ${selectedX.length} vars → ${yCol}`;
    }
    if (headers.length > 0) {
  ...
  return (
    <>
      ...
      <CollapsibleNodeWrapper
        nodeId={id}
        category="regression" nodeType="multiLinearRegression"
        title={data.label || "Multi Linear Req."}
        icon={<div style={{ display: 'flex', fontSize: '0.8em', fontWeight: 'bold' }}>MLR</div>}
        statusIndicator={getStatusIndicator()}
        infoButton={<InfoButton nodeType="multiLinearRegression" />}
      ...
      >
        {headers.length === 0 && (
          <div style={{
            ...
            <div>
              Connect a data source node to configure multi-linear regression.
            </div>
          </div>
        )}
...
export default React.memo(MultiLinearRegressionNode);