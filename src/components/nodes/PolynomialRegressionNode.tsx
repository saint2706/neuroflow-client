...
const PolynomialRegressionNode = ({ id, data, isConnectable }) => {
    ...
    const onRun = async () => {
        ...
        if (degree < 1 || degree > 5) {
            alert('Polynomial degree must be between 1 and 5.');
            return;
        }
        setIsTraining(true);
        try {
            ...
            // Call Python API
            setTrainMsg(`Training polynomial regression model (degree=${degree})...`);
            const result = await trainPolynomialRegression(
                X, Y, trainPercent, degree, includeBias, interactionOnly, selectedX, yCol
            );
            ...
            if (result.success) {
                    ...
                    return {
                        ...
                        data: {
                            ...
                            model: {
                                ...
                                interaction_only: result.interaction_only,
                ...
                // alert('Polynomial Regression training finished.');
            } else {
    ...
    const getCollapsedSummary = () => {
        if (modelResults) {
            ...
            return `Trained (Deg ${degree}) | R²: ${r2}%`;
        }
        if (selectedX.length > 0 && yCol) {
            return `Configured (Deg ${degree}) | ${selectedX.length}X → ${yCol}`;
        }
        if (headers.length > 0) {
    ...
    return (
        <>
            ...
            <CollapsibleNodeWrapper
                nodeId={id}
                category="regression" nodeType="polynomialRegression"
                title={data.label || 'Polynomial Regression'}
                icon={<div style={{ display: 'flex', fontSize: '0.8em', fontWeight: 'bold' }}>Poly</div>}
                statusIndicator={getStatusIndicator()}
                infoButton={<InfoButton nodeType="polynomialRegression" />}
            ...
            >
                {headers.length === 0 && (
                    <div style={{
                        ...
                        <div>
                            Connect a data source node to configure polynomial regression.
                        </div>
                    </div>
                )}
                    ...
                    <div className="lr-selects">
                        ...
                        <div className="lr-row">
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                <button
                                    ...
                                    onClick={() => {
                                        if (degree > 1) {
                                            const newVal = degree - 1;
                                            setDegree(newVal);
                                            setDegreeInput(newVal.toString());
                                        }
                                    }}
                                    disabled={degree <= 1}
                                    style={{
                                        ...
                                        borderRadius: '3px',
                                        background: '#f8f9fa',
                                        cursor: degree <= 1 ? 'not-allowed' : 'pointer'
                                    }}
                                >
                                    -
                                </button>
                                <input
                                    ...
                                    onBlur={(e) => {
                                        ...
                                        if (isNaN(numVal) || numVal < 1 || numVal > 5) {
                                            setDegreeInput(degree.toString());
                                        } else {
                                            setDegreeInput(numVal.toString());
                                            setDegree(numVal);
                                        }
                                    }}
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        borderRadius: '3px',
                                        background: '#f8f9fa',
                                        cursor: degree >= 5 ? 'not-allowed' : 'pointer'
                                    }}
                                >
                                    +
                                </button>
                        ...
                        <div className="lr-row">
                            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                                ...
                                />
                                <span>Include Interaction Terms Only</span>
                            </label>
                        </div>
                        ...
                        <div className="lr-row">
                            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                                ...
                                />
                                <span>Include Bias (Intercept)</span>
                            </label>
                        </div>
                            ...
                            <div className="model-results">
                                {degree > 3 && (
                                    <div style={{
                                        ...
                                        ⚠️ High polynomial degree ({degree}) may lead to overfitting
                                    </div>
                                )}
                                ...
                                <div>Degree: <strong>{modelResults.degree}</strong></div>
                                <div>Original Features: <strong>{modelResults.n_features_original}</strong></div>
                                <div>Polynomial Features: <strong>{modelResults.n_features_poly}</strong></div>
                                <div>Interaction Only: <strong>{modelResults.interaction_only ? 'Yes' : 'No'}</strong></div>
                                <div>Include Bias: <strong>{modelResults.include_bias ? 'Yes' : 'No'}</strong></div>
                                ...
                                <div className="equation-container">
                                    <div className="equation-display">
                                        {yCol} = {modelResults.coefficients.map((coef, i) => {
                                            const featureName = modelResults.feature_names?.[i] || `f${i}`;
                                            const sign = i === 0 ? '' : (coef >= 0 ? ' + ' : ' - ');
                                            const absCoef = Math.abs(coef).toFixed(4);

                                            // For bias term (feature name '1')
                                            if (featureName === '1') {
                                                return `${sign}${absCoef}`;
                                            }
                                            // For other features
                                            return `${sign}${absCoef}*${featureName}`;
                                        }).join('')}
                                    </div>
                                ...
                                <div style={{
                                    ...
                                    <div style={{ fontWeight: '600', marginBottom: '4px', color: '#475569' }}>Feature Legend:</div>
                                    {selectedX.map((colName, idx) => (
...
export default React.memo(PolynomialRegressionNode);
